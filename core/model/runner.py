...
class ModelRunner:
    ...
    def load(self) -> None:
        ...
        log.info(
            "model '%s': loading %s on %s  tracker=%s",
            self._cfg.id, self._cfg.path, self._device,
            self._tracker if self._use_tracker else "off",
        )
    ...
    def run(self, frame, active_classes: list[str]) -> list[InferenceResult]:
        return self._track(frame, active_classes) if self._use_tracker else self._predict(frame, active_classes)
    ...
    def _predict(self, frame, active_classes: list[str]) -> list[InferenceResult]:
        ...
        return self._parse(results, track=False)
    ...
    def _track(self, frame, active_classes: list[str]) -> list[InferenceResult]:
        ...
        return self._parse(results, track=True)
    ...
    def _parse(self, results, track: bool) -> list[InferenceResult]:
